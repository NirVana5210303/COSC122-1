...
class LinearHashTable():
    ...
    def store(self, item):
        ...
        slot_to_use = self._hash(item)
        if self._data[slot_to_use] is not None:
            slot_to_use = self._next_free_slot(slot_to_use, item)
        self._data[slot_to_use] = item
        # Keep track of number of items in hash table
        self.n_items += 1
...
#---------------------------------------------
def spellcheck_with_list(document, dict_list):
    """
    Checks the spelling of each word in 'document' (a list of words) against
    the dict_list (another list of words).
    """
    ...
    while index < len(document):
        if document[index] not in dict_list:
            print(document[index])
            num_errors +=1
            unique_words.add(document[index])
        index+=1
...
def spellcheck_with_hashtable(document, dictionary, ht_type, ht_size):
    ...
    # ***********************************************************
    # Write your code to fill the hash table here
    # ---start student section---
    ...
    unique_words = set()
    while index < len(document):    
        if document[index] not in hash_table:
            print(document[index])
            num_errors +=1
            unique_words.add(document[index])
            if num_errors == 100:
                print(document[index],"YOLO 100th word :)")  
        index+=1
    print(len(unique_words))    
