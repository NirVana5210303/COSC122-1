...
class KochSnowflake(KochCurve):
	...
	NOTE: You need to change the angle for the Koch curve to 80 for this to work"""
    ...
    def draw(self, width, height):
        ...
        if self.levels <= 2:
            length = math.sqrt(math.pow(width/3, 2)+
                               math.pow(height/3, 2))
        else:
            length = math.sqrt(math.pow(width/self.levels, 2)+
                               math.pow(height/self.levels, 2))
        # Now draw three kochcurves, turning 120o right in between
        # ---start student section---
        pass
        # ===end student section===
